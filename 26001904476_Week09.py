...
def initPopulation():
    ...
    for i in range(populationSize):  #Check population size
        initPop.append(''.join(
            random.choice(string.printable) for i in range(dnaLen)))
    return initPop
...
def weightedDNAchoice(competingDNAfitnessPairs):
    probs = [
        competingDNAfitnessPairs[i][1]
        for i in range(len(competingDNAfitnessPairs))
    ]
    probs = np.array(probs)
    probs /= probs.sum()
    return competingDNAfitnessPairs[np.random.choice(
        len(competingDNAfitnessPairs), 1, p=probs)[0]][0]
...
for i in range(generations):
    ...
    print("The last fittest DNA", i, "is---",
          currentpop[lastfitnessarray.index(min(lastfitnessarray))],
...
print("fitesst string at: ", generations, " is",
      currentpop[lastfitnessarray.index(min(lastfitnessarray))])
